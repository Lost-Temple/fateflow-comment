...
class TrackerClient(object):
    def __init__(self, job_id: str, role: str, party_id: int,
        ...
        self.component_name = component_name if component_name else 'pipeline'  # 如果component_name不为空，就用component_name，否则用pipeline(首字母小写)
        self.module_name = component_module_name if component_module_name else 'Pipeline'  # 如果component_module_name不为空，就用component_module_name，否则为Pipeline(首字母大写）
